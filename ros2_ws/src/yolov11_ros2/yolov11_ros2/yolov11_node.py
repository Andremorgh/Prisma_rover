...
class Yolov11Node(Node):
    ...
    def __init__(self):
        ...
        self._color_image_sub = self.create_subscription(Image, "color/image_raw", self.color_image_callback, qos_profile_image)
        self._depth_image_sub = self.create_subscription(Image, "depth/image_raw", self.depth_image_callback, qos_profile_image)
        self._camera_info_subscriber = self.create_subscription(CameraInfo, 'color/camera_info', self.camera_info_callback, QoSProfile(depth=1,reliability=ReliabilityPolicy.RELIABLE))
        self.bridge = CvBridge()

    def color_image_callback(self, msg: Image):
        self.color_image_msg = msg
        
        # Rate limit YOLO inference to ~1.2 Hz (0.8s minimum gap)
    ...
    def object_segmentation(self):
        if self.enable_yolo and self.color_image_msg is not None and self.depth_image_msg is not None:
            ...
            results = self.yolo.predict(
                ...
                classes=[24, 39, 32, 77], #[ 9, 10, 58, 77], #oggetti Leonardo: traffic light, fire hydrant, potted plant , teddy bear
                show=False,
                verbose=False,
                stream=False,
                conf=self.threshold,
            ...
            for detection in results:
                ...
                self.pred_compressed_msg.header.frame_id = "prisma_rover/camera_rgb_optical_frame"  # Aggiungi frame_id opzionale
    ...
    def process_yolo_detections(self, color_img_msg, depth_img_msg, detections):
        ...
        pose_array.header.frame_id = 'prisma_rover/camera_rgb_optical_frame'
