...
from prisma_rover_interfaces.msg import DetectedObjectsList
...
from prisma_rover_obj_det_agent.two_conv_nets import *
...
class AgentNode(Node):
    ...
    def __init__(self, use_learned_policy=False):
        ...
        # Robust path resolution for model and vocab
        def resolve_resource_path(filename):
            # Try script directory
            path = os.path.join(os.path.dirname(__file__), filename)
            if os.path.exists(path):
                return path
            # Try prisma_rover_obj_det_agent package dir
            try:
                import prisma_rover_obj_det_agent
                pkg_dir = os.path.dirname(prisma_rover_obj_det_agent.__file__)
                path = os.path.join(pkg_dir, filename)
                if os.path.exists(path):
                    return path
            except ImportError:
                pass
            # Try hardcoded workspace absolute fallbacks
            ws_path_obj = f"/home/andrea/Desktop/Quantum_obj_rover/ros2_ws/src/prisma_rover_obj_det_agent/prisma_rover_obj_det_agent/{filename}"
            if os.path.exists(ws_path_obj):
                return ws_path_obj
            return filename

        model_path = resolve_resource_path("agent_1747644302.cleanrl_model")
        vocab_path = resolve_resource_path("vocab.json")

        self.agent.load_state_dict(torch.load(model_path, map_location=self.device), strict=False)
        self.agent.eval()

        self.vocab = Vocabulary(vocab_max_size)
        self.vocab.load_from_json(vocab_path)
        ...
        self.obj_sub = self.create_subscription(DetectedObjectsList, objects_topic, self.objects_callback, 1, callback_group=callback_group)
        self.map_sub = self.create_subscription(OccupancyGrid, map_topic, self.map_callback, 1, callback_group=callback_group)
        self.pose_sub = self.create_subscription(PoseStamped, pose_topic, self.pose_callback, 1, callback_group=callback_group)
        ...
        # Move base client
        self.client = ActionClient(self, NavigateToPose, nav_action)
